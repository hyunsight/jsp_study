...
//model(모델):
//1. 직접적으로 Database와 대화를 한다. (즉, 데이터를 저장, 수정, 삭제, 조회(select) 요구)
...
public class ProductService {
	...
	public List<Product> findAll() {
		//product 객체가 저장된 hashmap을 arrayList로 변경 후 리턴
		return new ArrayList<>(products.values()); //product 객체가 담긴 arrayList 리턴
	}
	
	//상품코드로 상품 객체를 리턴해주는 메소드
	public Product find(String id) {
		return products.get(id); //상품코드(key)를 이용해서 product 객체를 받아온다.
	}
}
